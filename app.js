...
const LEAD_ENDPOINT = "https://script.google.com/macros/s/AKfycbyiM2ELf3wY5MP6lzkGl4cZMl7uWSVD8y2K1Z9xT2LjYqfn1RYbjvcf2Wwjk9t5UZ2zjA/exec";
...
function renderContact(step) {
  ...
  app.innerHTML=frame(`<p class="question-kicker">${step.kicker}</p><h1 class="question-title">${step.title}</h1><p class="question-subtitle">${step.subtitle}</p><div class="form-grid"><div class="field full"><label for="name">Seu nome</label><input id="name" autocomplete="name" value="${esc(a.name)}" placeholder="Como podemos chamar você?"></div><div class="field"><label for="phone">WhatsApp</label><input id="phone" inputmode="tel" autocomplete="tel" value="${esc(a.phone)}" placeholder="(31) 99999-9999"></div><div class="field"><label for="email">E-mail <span style="font-weight:400;color:var(--muted)">(opcional)</span></label><input id="email" type="email" value="${esc(a.email)}" placeholder="voce@email.com"></div><div class="honeypot" aria-hidden="true"><label for="website">Não preencha</label><input id="website" tabindex="-1" autocomplete="off"></div><label class="consent full"><input id="consent" type="checkbox" ${a.consent?"checked":""}><span>Concordo com o uso das minhas respostas e dados de contato para receber este diagnóstico e permitir o atendimento da Vila La Vie.</span></label><p class="field-error full" id="contact-error" hidden>Informe seu nome, um WhatsApp válido e marque o consentimento.</p></div><div class="actions"><button class="back" data-back>← Voltar</button><button class="primary" data-contact>Continuar →</button></div>`);
...
async function submitLead(result) {
  if(state.submission!=="idle")return;
  state.submission="sending";
  const c=state.answers.contact,w=state.answers.wedding,p=state.answers.planning,t=tracking();
  const status=document.querySelector("[data-submission]");
  const payload={lead_id:state.leadId,name:c.name,phone:c.phone,email:c.email,wedding_date:w.date,city:w.city,period:w.period,venue:w.venue,primary_profile:profiles[result.primary].name,secondary_profile:profiles[result.secondary].name,classica:result.percentages.classica,romantica:result.percentages.romantica,contemporanea:result.percentages.contemporanea,marcante:result.percentages.marcante,service:p.service,budget:p.budget,size:p.size,availability:p.availability,utm_source:t.source,utm_campaign:t.campaign,utm_content:t.content,page_url:location.href,consent:c.consent,website:c.website};
  try {
    ...
    if(status)status.textContent="Diagnóstico recebido pela equipe Vila La Vie.";
  } catch (_) {
    state.submission="failed";
    if(status)status.textContent="Seu resultado está pronto. Você também pode enviá-lo pelo WhatsApp.";
  }
}
...
function renderResult() {
  ...
  app.innerHTML=`<div class="quiz-shell">${header()}<main class="result-page"><div class="result-wrap">
    ...
    <section class="result-section"><div class="cta"><div><p class="section-kicker">Próximo passo</p><h2 class="section-heading">Viva a experiência Vila La Vie</h2><p>Envie seu perfil para uma consultora. As informações do diagnóstico já seguem organizadas para facilitar o início da sua curadoria.</p><small class="lead-status" data-submission>Salvando seu diagnóstico com segurança...</small></div><div class="cta-actions"><a class="whatsapp" href="${whatsappUrl(result)}" target="_blank" rel="noopener noreferrer">Conversar pelo WhatsApp</a><button class="secondary" data-pdf>Salvar resultado em PDF</button></div></div><p class="disclaimer">O diagnóstico é uma orientação inicial de estilo e não representa reserva de horário, disponibilidade de modelo ou proposta comercial. Com o consentimento informado, suas respostas e dados de contato são enviados à Vila La Vie para viabilizar o atendimento.</p><button class="restart" data-restart>Refazer diagnóstico</button></section>
  </div></main></div>`;
  submitLead(result);
}
...
function bind() {
  ...
  document.querySelector("[data-contact]")?.addEventListener("click",()=>{const name=document.querySelector("#name").value.trim(),phone=document.querySelector("#phone").value.trim(),email=document.querySelector("#email").value.trim(),consent=document.querySelector("#consent").checked,website=document.querySelector("#website").value;if(name.length<3||!validPhone(phone)||!consent){document.querySelector("#contact-error").hidden=false;return;}state.answers.contact={name,phone,email,consent,website};state.current++;render();});
