...
function whatsappUrl(result) {
  const c=state.answers.contact,w=state.answers.wedding,p=state.answers.planning,profile=profiles[result.primary];
  const message=["Olá, Vila La Vie! Fiz o Diagnóstico do Vestido Ideal e gostaria de conversar sobre um atendimento.","",`Nome: ${c.name}`,`WhatsApp: ${c.phone}`,`Perfil: ${profile.name}`,`Casamento: ${formatDate(w.date)} | ${w.city}`,`Cenário: ${w.period} | ${w.venue}`,`Interesse: ${p.service}`,`Investimento planejado: ${p.budget}`,`Numeração aproximada: ${p.size}`,`Disponibilidade: ${p.availability}`,"","Podemos continuar por aqui?"].join("\n");
...
function renderResult() {
  ...
  app.innerHTML=`<div class="quiz-shell">${header()}<main class="result-page"><div class="result-wrap"><section class="result-hero"><div class="result-copy"><p class="kicker">Seu perfil, ${esc(c.name.split(" ")[0])}</p><h1 class="result-title">${main.name}</h1><p class="result-description">${main.description}</p></div><div class="result-photo"><img src="./assets/noiva-vestido-2.jpg" alt="Vestido Vila La Vie"></div></section><section class="result-section"><h2 class="section-heading">A direção que mais combina com você</h2><p class="section-lead">Seu resultado é um ponto de partida para a prova, não uma regra. A consultoria existe justamente para transformar referências em uma escolha que faça sentido no seu corpo e no seu casamento.</p><div class="profile-grid"><article class="profile-card"><span>Silhuetas para começar</span><h3>Forma e proporção</h3><p>${main.silhouette}</p></article><article class="profile-card"><span>Tecidos e detalhes</span><h3>O olhar de perto</h3><p>${main.details}</p></article><article class="profile-card"><span>Durante a prova</span><h3>Como escolher melhor</h3><p>${main.fitting}</p></article></div></section><section class="result-section"><h2 class="section-heading">Sua composição de estilo</h2><p class="section-lead">Além da essência ${main.short.toLowerCase()}, há uma influência ${secondary.short.toLowerCase()} que pode aparecer nos detalhes e acessórios.</p><div class="match-list">${matches}</div></section><section class="result-section"><h2 class="section-heading">Informações para sua curadoria</h2><div class="lead-summary"><div><span>Casamento</span><strong>${esc(formatDate(w.date))}</strong></div><div><span>Cidade</span><strong>${esc(w.city)}</strong></div><div><span>Interesse</span><strong>${esc(p.service)}</strong></div><div><span>WhatsApp</span><strong>${esc(c.phone)}</strong></div></div></section><section class="result-section"><div class="cta"><div><h2 class="section-heading">Viva a experiência Vila La Vie</h2><p>Envie seu perfil para uma consultora e converse sobre a melhor forma de começar sua curadoria com horário reservado.</p></div><div class="cta-actions"><a class="whatsapp" href="${whatsappUrl(result)}" target="_blank" rel="noopener noreferrer">Conversar pelo WhatsApp</a><button class="secondary" data-pdf>Salvar resultado em PDF</button></div></div><p class="disclaimer">O diagnóstico é uma orientação inicial de estilo e não representa reserva de horário, disponibilidade de modelo ou proposta comercial. Seus dados só seguem para a Vila La Vie quando você envia a mensagem pelo WhatsApp.</p><button class="restart" data-restart>Refazer diagnóstico</button></section></div></main></div>`;
}
